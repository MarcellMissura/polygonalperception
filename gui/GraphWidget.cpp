...
Vec2 GraphWidget::getLeftRightStateIndex()
{
	...
	int startIndex = stateUtil.findIndex(boundingBox.left());
	int endIndex = stateUtil.findIndex(boundingBox.right());

    return Vec2(startIndex, endIndex);
}

// Writes a gnuplot ready data export to data/export.txt from the data that is currently
// displayed in the graph widget.
...
// Sets the time offset t that defines what value in the time stream state.time
// corresponds to the 0 value of the x axis.
void GraphWidget::frameIndexChangedIn(int cfi)
{
    this->t = state[cfi].time;
	update();
}
...
void GraphWidget::mouseMoveEvent(QMouseEvent *event)
{
	...
	if (draggedCurveId == -1)
	{
		...
		foreach (int id, idsOfTheCurvesToShow)
		{
			...
			double y = curve.interpolatedTransformedValueAt(mappedMouse.x() + t);
	...
	if (event->buttons() & (Qt::LeftButton | Qt::RightButton | Qt::MiddleButton))
	{
		...
		if (draggedCurveId > -1)
		{
			...
			{
				curves[draggedCurveId].stretchBy(draggedCurveX + t, mappedMouseDiff.y()); // + t because the curves are offset by -t
			}
			else
			{
		...
		else if (event->modifiers() & Qt::ControlModifier || event->buttons() & (Qt::RightButton | Qt::MiddleButton))
		{
            ...
            //qDebug() << screenScale << "mapped mouse" << mappedMouse << "mouse diff:" << mappedMouseDiff;
			updateScreenTransform();
		}
...
void GraphWidget::mouseReleaseEvent(QMouseEvent *event)
{
	...
	{
		curves[nearestCurveId].setMarkerAt(mappedMouse.x()+t);
	}
...
void GraphWidget::paintEvent(QPaintEvent*)
{
	...
	if (mousePresent)
	{
        ...
        painter.setPen(pen);
		x = qBound(state[state.size()-1].time, mappedMouse.x()+t, state.time);
	...
	foreach (int id, idsOfTheCurvesToShow)
	{
		...
		if (mousePresent)
		{
			x = qBound(state[state.size()-1].time-t, mappedMouse.x(), state.time-t);
			y = curves[id].transformedValueAt(mappedMouse.x()+t);
			mp = screenTransform.map(QPointF(x, y)) + QPointF(10,-4);
			mp.ry() = qBound((double)(fontHeight), mp.y(), (double)(height()-1));
			painter.drawText(mp, QString::number(curves[id].valueAt(mappedMouse.x()+t), 'f', 3) );
		}

		// Draw the zero marker value.
		x = 0;
		y = curves[id].interpolatedTransformedValueAt(t);
		mp = screenTransform.map(QPointF(x, y)) + QPointF(10,-4);
		mp.ry() = qBound((double)(fontHeight), mp.y(), (double)(height()-1));
		painter.drawText(mp, QString::number(curves[id].interpolatedValueAt(t), 'f', 3) );
	}
