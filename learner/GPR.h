...
public:
	...
	int size();
    void addDataPoint(const NVec<DIM> &x, const double &y);
...
private:
    ...
    GridNet<DIM> meanGrid; // Rasterized mean values.
    GridNet<DIM> stddevGrid; // Rasterized stddev values.
...
template <int DIM>
void GPR<DIM>::addDataPoint(const NVec<DIM> &x, const double &y)
{
...
template <int DIM>
void GPR<DIM>::addData(QList< NVec<DIM> > x, QList<double> y)
{
...
{
    meanGrid.drawGridSlice(z, sampleFactor, zScale, colorContrast, showData);
}
