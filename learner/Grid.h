...
protected:
    ...
    std::vector<double> max; // Input range max values.
    std::vector<double> min; // Input range min values.
    ...
    std::vector<double> stride; // speeds up evaluateAt()
...
public:
    ...
    QVector<uint> getEnvelopingNodes(const double* x, uint radius=0) const; // Returns the flat indices of the nodes surrounding the query point.
    QVector<uint> enumerateNeighborHood(const uint* idx, uint radius=1) const; // Returns a list of flat indexes of the neighbors of node idx within radius r.
    QVector<uint> enumerateNeighborHood(uint idx, uint radius=1) const; // Returns a list of flat indexes of the neighbors of node idx within radius r.
